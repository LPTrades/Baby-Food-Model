...
# ─────────────────────────────────────────────
# 15-MINUTE TIME SLOT BREAKDOWN
# Buckets every trade by entry time into 15m windows
# spanning the full session (sess_start_t → sess_end_t)
# ─────────────────────────────────────────────
...
sess_start_min = time_to_minutes(sess_start_t)
sess_end_min   = time_to_minutes(sess_end_t)

# Build slot labels: every 15m from sess_start to sess_end
slots = []
cursor = sess_start_min
while cursor < sess_end_min:
    slot_end = min(cursor + 15, sess_end_min)
    h1, m1 = divmod(cursor,   60)
    h2, m2 = divmod(slot_end, 60)
    label = f"{h1:02d}:{m1:02d}–{h2:02d}:{m2:02d}"
    slots.append((cursor, slot_end, label))
    cursor += 15

# Bin trades into slots
slot_trades = {label: [] for _, _, label in slots}
for t in trades:
    entry_min = time_to_minutes(t["entry_time"].time())
    for (s_min, e_min, label) in slots:
        if s_min <= entry_min < e_min:
            slot_trades[label].append(t)
            break

# Build output
lines.extend(["", "=" * 50,
              f"  ENTRY TIME SLOT BREAKDOWN  (15-min windows)",
              "=" * 50])
lines.append(f"  {'Slot':<16} {'Trades':>6}  {'W':>5}  {'L':>5}  {'WR':>7}  {'Exp R':>7}")
lines.append("  " + "-" * 54)

slot_summary_rows = []
for _, _, label in slots:
    tlist = slot_trades[label]
    n_slot = len(tlist)
    if n_slot == 0:
        lines.append(f"  {label:<16} {'–':>6}  {'–':>5}  {'–':>5}  {'–':>7}  {'–':>7}")
        slot_summary_rows.append((label, 0, 0, 0, None, None))
        continue
    w  = sum(1 for t in tlist if t["win"])
    l  = n_slot - w
    wr = w / n_slot * 100
    exp = sum(t["r"] for t in tlist) / n_slot
    lines.append(f"  {label:<16} {n_slot:>6}  {w:>5}  {l:>5}  {wr:>6.1f}%  {exp:>+7.3f}R")
    slot_summary_rows.append((label, n_slot, w, l, wr, exp))

# Print the same breakdown to console
print("\n" + "=" * 56)
print("  ENTRY TIME SLOT BREAKDOWN  (15-min windows)")
print("=" * 56)
print(f"  {'Slot':<16} {'Trades':>6}  {'W':>5}  {'L':>5}  {'WR':>7}  {'Exp R':>7}")
print("  " + "-" * 54)
for (label, n_slot, w, l, wr, exp) in slot_summary_rows:
    if n_slot == 0:
        print(f"  {label:<16} {'–':>6}  {'–':>5}  {'–':>5}  {'–':>7}  {'–':>7}")
    else:
        print(f"  {label:<16} {n_slot:>6}  {w:>5}  {l:>5}  {wr:>6.1f}%  {exp:>+7.3f}R")
print("=" * 56)
