# Version Update - 2026-06-09

## Changes Added
- Optimized output for different RR
- Added a new way of taking profit. Trades can only close if they reverse into another trade or 12:00 NY time hits, otherwise they hit stop loss. This can be activated through an input.
...
EXIT RULE (this version):

  NO fixed take-profit target.

  A trade stays open until one of three things happens:

    1. SL is hit (original stop-loss logic unchanged).

    2. Session close at 12:00 NY time.

    3. A full opposite CISD+FVG entry signal fires on the current bar.

       The open trade closes at bar close, and the new opposite trade

       opens on the same bar at the same price.

  This means winners can run as far as the next structural reversal allows,

  and the RR on any given trade is open-ended (recorded as actual R).
...
tp_raw   = prompt("TP mode (fixed/structural)",  "structural").strip().lower()

tp_mode  = tp_raw if tp_raw in ("fixed", "structural") else "structural"

rr_input = float(prompt("RR target (used when tp_mode=fixed)", "1")) if tp_mode == "fixed" \

           else 1.0   # unused in structural mode but kept for chart label consistency
...
print(f"  TP mode: {tp_mode}" + (f"  RR: {rr_input:g}" if tp_mode == "fixed" else ""))
...
force_close_time = dtime(12, 0)   # session close — no new entries after this either
...
        # fixed TP check (only runs when tp_mode == "fixed")

        if tp_mode == "fixed" and tp is not None:

            hit_tp = (d == "long"  and bar["high"] >= tp) or \

                     (d == "short" and bar["low"]  <= tp)
...
        hit_sl = (d == "long"  and bar["low"]  <= sl) or \

                 (d == "short" and bar["high"] >= sl)
...
        # While in a trade, keep CISD/FVG tracking live for the OPPOSITE side only.

        # This lets an opposing entry signal build without disturbing the open trade.

  

        # ── BEARISH CISD (tracking while long) ──

        if d == "long":

            if bias != BIAS_LONG and (recent_sw_high is not None or bias == BIAS_SHORT):
...
                        bull_run_open  = df1.iloc[ts]["open"]
...
            # ── 5m break detection while long ──

            if recent_sw_high and bar["high"] > recent_sw_high["level"]:
...
            # ── Opposite entry fires → close long, open short ──
...
                        r_long = (ep - active_trade["entry"]) / active_trade["risk"]

                        active_trade.update(exit_price=ep, exit_time=bar_dt,

                                            result="opp", r=r_long, win=(r_long > 0))
...
                            tp          = (entry - risk_s * rr_input) if tp_mode == "fixed" else None,
...
        # ── BULLISH CISD (tracking while short) ──

        if d == "short":

            if bias != BIAS_SHORT and (recent_sw_low is not None or bias == BIAS_LONG):
...
                        bear_run_open  = df1.iloc[ts]["open"]
...
            # ── 5m break detection while short ──
...
            # ── Opposite entry fires → close short, open long ──
...
                        r_short = (active_trade["entry"] - ep) / active_trade["risk"]

                        active_trade.update(exit_price=ep, exit_time=bar_dt,

                                            result="opp", r=r_short, win=(r_short > 0))
...
                            tp          = (entry + risk_l * rr_input) if tp_mode == "fixed" else None,
...
    # CONTINUOUS CISD TRACKING (flat only)
...
                    tp          = (entry + risk * rr_input) if tp_mode == "fixed" else None,
...
# break down exit types

n_sl  = sum(1 for t in trades if t.get("result") == "sl")

n_opp = sum(1 for t in trades if t.get("result") == "opp")

n_eod = sum(1 for t in trades if t.get("result") == "eod")

print(f"  SL: {n_sl}  |  Opposite-entry flip: {n_opp}  |  EOD: {n_eod}")
...
    gp     = sum(t["r"] for t in tlist if t["r"] > 0)

    gl     = sum(abs(t["r"]) for t in tlist if t["r"] < 0)

    pf     = gp / gl if gl else float("inf")

    exp    = sum(t["r"] for t in tlist) / n

    rs     = sorted(t["r"] for t in tlist)
...
                med_r=med_r, mw=mw, ml=ml, sim=bal)
...
row("TP mode",                tp_mode + (f"  RR={rr_input:g}" if tp_mode == "fixed" else ""))

row("Exit rule",              "SL / opp-entry flip / 12:00 NY")

row("Total Trades",           s["n"])

row("SL / Opp-flip / EOD",   f"{n_sl} / {n_opp} / {n_eod}")
...
    ax.set_title("Equity Curve  (tp=" + tp_mode + (f" RR={rr_input:g}" if tp_mode == "fixed" else "") + " · close 12:00 NY · 1% risk · $10,000 start)",
...
            "Exit Time":        pd.Timestamp(t.get("exit_time","")).strftime("%Y-%m-%d %H:%M") if t.get("exit_time") else "",
...
            "Exit Price":       round(t.get("exit_price",0),5),
