...
# ─────────────────────────────────────────────
def reset_bear_run():
    global bear_run_start, bear_run_count, bear_run_open, bear_run_sl
    bear_run_start = None; bear_run_count = 0
    bear_run_open  = None; bear_run_sl    = None

def reset_bull_run():
    global bull_run_start, bull_run_count, bull_run_open, bull_run_sl
    bull_run_start = None; bull_run_count = 0
    bull_run_open  = None; bull_run_sl    = None

def reset_long_cisd():
    global cisd_long_start, cisd_long_end, cisd_long_high, cisd_long_low, cisd_long_sl
    global fvg_long_valid, fvg_long_stored
    cisd_long_start = cisd_long_end = cisd_long_high = cisd_long_low = cisd_long_sl = None
    fvg_long_valid  = False
    fvg_long_stored = None

def reset_short_cisd():
    global cisd_short_start, cisd_short_end, cisd_short_high, cisd_short_low, cisd_short_sl
    global fvg_short_valid, fvg_short_stored
    cisd_short_start = cisd_short_end = cisd_short_low = cisd_short_high = cisd_short_sl = None
    fvg_short_valid  = False
    fvg_short_stored = None
...
# ─────────────────────────────────────────────
def scan_fvg_long(cisd_start, i1):
    n = len(df1)
    for c2_idx in range(cisd_start, i1):
        c1_idx = c2_idx - 1
        c3_idx = c2_idx + 1
...
def scan_fvg_short(cisd_start, i1):
    n = len(df1)
    for c2_idx in range(cisd_start, i1):
        c1_idx = c2_idx - 1
        c3_idx = c2_idx + 1
...
for i1 in range(n1):
    ...
    # ── BULLISH CISD ──
    if (recent_sw_low is not None or bias == BIAS_LONG) and not in_trade and not setup_used:
        if is_bearish(bar):
            prev_bearish = (i1 > 0 and is_bearish(df1.iloc[i1-1]))
            if bear_run_start is None or not prev_bearish:
                # New sequence starting — reset run tracker only
                # Do NOT wipe CISD range yet; wait to see if new extreme forms
                bear_run_start = i1
                bear_run_open  = bar["open"]
                bear_run_sl    = bar["close"]
                bear_run_count = 1
            else:
                bear_run_count += 1
            ...
            if bear_run_count >= 2:
                if cisd_long_low is None or bar["low"] < cisd_long_low:
        ...
        # FVG scan — persists until range makes new extreme
        if cisd_long_start is not None and not fvg_long_valid:
            fvg = scan_fvg_long(cisd_long_start, i1)
            if fvg:
                fvg_long_stored = fvg
                fvg_long_valid  = True

    # ── BEARISH CISD ──
    if (recent_sw_high is not None or bias == BIAS_SHORT) and not in_trade and not setup_used:
        if is_bullish(bar):
            prev_bullish = (i1 > 0 and is_bullish(df1.iloc[i1-1]))
            if bull_run_start is None or not prev_bullish:
                bull_run_start = i1
                bull_run_open  = bar["open"]
                bull_run_sl    = bar["close"]
                bull_run_count = 1
            else:
                bull_run_count += 1
            ...
            if bull_run_count >= 2:
                if cisd_short_high is None or bar["high"] > cisd_short_high:
                    # New extreme — update range and wipe FVG
                    cisd_short_start = i1
                    cisd_short_end   = i1
                    cisd_short_low   = bull_run_open   # locked to first candle open
                    cisd_short_high  = bar["high"]
        ...
        if cisd_short_start is not None and not fvg_short_valid:
            fvg = scan_fvg_short(cisd_short_start, i1)
            if fvg:
